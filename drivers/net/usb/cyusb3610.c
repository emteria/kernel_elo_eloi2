/*
 * Cypress CYUSB3610 based USB 3.0 Ethernet Devices
...
#endif
#include <linux/debugfs.h> // 2017/02/21, PP FTD EthLED mode
#include <soc/qcom/socinfo.h> // 2017/02/22, Get MAC from OEM QOTP
#include "cyusb3610.h"
#include "command.h"

#define DRV_VERSION	"1.17.101"
...
/* Cypress GX3 based USB 3.0/2.0 Gigabit Ethernet Devices */

/* 2017/02/22, hardcode led setting {*/
struct dentry *led_root;
static int hardcode_flag = 1;
u16 hardcode_ledvalue;
/* 2017/02/22, hardcode led setting }*/

/* 2017/02/20, PP FTD Get ETHERNET ID {*/
static char PID[5];
static char VID[5];
/* 2017/02/20, PP FTD Get ETHERNET ID }*/
...
static int cyusb3610_mdio_read(struct net_device *netdev, int phy_id, int loc)
{
	...
	cyusb3610_read_cmd(dev, CY_ACCESS_PHY, phy_id, (__u16)loc, 2, tmp16, 1);
		   ...
		   u8 *data)
{
	...
	/* GX3 returns 2 bytes from eeprom on read */
...
/* 2017/01/15, QV1 add ethtool set EEPROM interface {*/
static int
qisda_cyusb3610_set_eeprom(struct net_device *net, struct ethtool_eeprom *eeprom,
		   u8 *data)
{
	struct usbnet *dev = netdev_priv(net);
    int ret;
    ...
    printk("Debug: EEPROM setting begin.\n");
	if (eeprom->len == 0)
		return -EINVAL;

    printk("Debug: eeprom data = %04x\n", *((u16 *)test));
    printk("Debug: eeprom magic = %x\n", eeprom->magic);
    printk("Debug: eeprom offset = %02x\n", eeprom->offset);

    if (eeprom->magic != CYUSB3610_EEPROM_MAGIC) {
		netdev_err(dev->net, "Debug: EEPROM magic value mismatch: 0x%x\n",
			    eeprom->magic);
		return -EINVAL;
	}
	wd = cpu_to_le16(*(test));
    printk("Debug: current wd = %04x\n", wd);
	ret = cyusb3610_write_cmd(dev, CY_ACCESS_EEPROM, eeprom->offset, 1, 2, &wd);
	mdelay(15);
    ...
    printk("Debug: set eeprom result = %d\n", ret);
    return ret;
}
/* 2017/01/15, QV1 add ethtool set EEPROM interface }*/
...
int ioctl_signature(struct usbnet *dev, CY_IOCTL_COMMAND *info)
{
	strncpy(info->sig, CYUSB3610_SIGNATURE, strlen(CYUSB3610_SIGNATURE));
	return 0;
}

#if 0
int ioctl_read_eeprom(struct usbnet *dev, CY_IOCTL_COMMAND *info)
{
	int *i;
	unsigned short *tmp;
	u8 *value;
	u8 *tmp_8;
	tmp_8 = kmalloc(7, GFP_KERNEL);
	if (!tmp_8)
		return -ENOMEM;
	value = (u8*)tmp_8;
	tmp = (unsigned short *)(&tmp_8[1]);
	i = (int *)(&tmp_8[3]);

	if (info->type == 0) //EEPROM
	{
		for (*i = 0; *i < info->size; (*i)++) {
			//retval = cyusb3610_read_cmd(dev, CY_ACCESS_EEPROM, i, 1, 2, &tmp);

			if (cyusb3610_write_cmd(dev, CY_ACCESS_MAC, CY_SROM_ADDR, 1, 1, i) < 0) {
				kfree(tmp_8);
				return -EINVAL;
			}

			*value = EEP_RD;
			if (cyusb3610_write_cmd(dev, CY_ACCESS_MAC, CY_SROM_CMD, 1, 1, value) < 0) {
				kfree(tmp_8);
				return -EINVAL;
			}
			...
			do
			{
				cyusb3610_read_cmd(dev, CY_ACCESS_MAC, CY_SROM_CMD, 1, 1, value, 0);
			} while (*value & EEP_BUSY);

			if (cyusb3610_read_cmd(dev, CY_ACCESS_MAC, CY_SROM_DATA_LOW, 2, 2, tmp, 0) < 0)  {
				kfree(tmp_8);
				return -EINVAL;
			}
	...
	else //eFuse
	{
		for (*i = 0; *i < info->size; (*i)++){
			...
			if (cyusb3610_read_cmd(dev, CY_ACCESS_EFUSE, *i, 1, 2, tmp, 0) < 0) {
				kfree(tmp_8);
				return -EINVAL;
			}
			*(info->buf + *i) = be16_to_cpu(*tmp);
...
int ioctl_write_eeprom(struct usbnet *dev, CY_IOCTL_COMMAND *info)
{
	int i;
	u16 *data,csum=0;
		...
		*(info->buf + 5) = *data;
		...
		for (i = 0; i < info->size; i++){
			*data = cpu_to_be16(*(info->buf + i));
			if (cyusb3610_write_cmd(dev, CY_ACCESS_EEPROM, i, 1, 2, data) < 0) {
				kfree(data);
				return -EINVAL;
			}
		}
	...
	else if (info->type == 1)  //eFuse
	{
		...
		*(info->buf + 0x19) = *data;
		...
		if (cyusb3610_write_cmd(dev, CY_WRITE_EFUSE_EN, 0, 0, 0, NULL) < 0) {
			kfree(data);
			return -EINVAL;
		}
		...
		for (i = 0; i < info->size; i++){
			*data = cpu_to_be16(*(info->buf + i));
			if (cyusb3610_write_cmd(dev, CY_ACCESS_EFUSE, i, 1, 2, data) < 0) {
				kfree(data);
				return -EINVAL;
			}
		}

		if (cyusb3610_write_cmd(dev, CY_WRITE_EFUSE_DIS, 0, 0, 0, NULL) < 0) {
			kfree(data);
			return -EINVAL;	
		}
	}
	else if (info->type == 2)  //check efuse existed or not
	{
		...
		if (cyusb3610_read_cmd(dev, CY_ACCESS_EFUSE, 0, 1, 2, data, 0) < 0) {
			kfree(data);
			return -EINVAL;
		}
...
int ioctl_ieee_test(struct usbnet *dev, CY_IOCTL_COMMAND *info)
{
	int 	i = 0;
	bool 	disable = info->type & IEEE_DISABLE;
	u16	nRegAddr[7] = {31, 5, 6, 31, 30, 21, 31};
	u16	*dwData;

	dwData = kmalloc(14, GFP_KERNEL);
	if (!dwData)
		return -ENOMEM;
	dwData[0] = 0x0005;
	dwData[1] = 0x8b86;
	...
	info->type &= IEEE_TYPEMASK;

	if ((info->type == IEEE_10TER) ||  (info->type == IEEE_10TEFF)
				|| (info->type == IEEE_10TEF0) || disable)
		goto NOINIT;

	for (i = 0; i < 7 ;i++) {
		if (cyusb3610_write_cmd(dev, CY_ACCESS_PHY, CYUSB3610_PHY_ID, nRegAddr[i], 2, &dwData[i]) < 0) {
			kfree(dwData);
			return -EINVAL;
		}
	}
...
NOINIT:
	if (info->type < IEEE_100CA) {
		nRegAddr[0] = 31; nRegAddr[1] = 9;
		dwData[0] = 0;
		if (disable)
			dwData[1] = 0;
		else if (info->type == IEEE_1000M1)
			dwData[1] = 0x2000;
		else if (info->type == IEEE_1000M4)
			dwData[1] = 0x8000;
		if (cyusb3610_write_cmd(dev, CY_ACCESS_PHY, CYUSB3610_PHY_ID, nRegAddr[0], 2, &dwData[0]) < 0) {
			kfree(dwData);
			return -EINVAL;
		}
		if (cyusb3610_write_cmd(dev, CY_ACCESS_PHY, CYUSB3610_PHY_ID, nRegAddr[1], 2, &dwData[1]) < 0) {
			kfree(dwData);
			return -EINVAL;
		}
	}
	else if (info->type > IEEE_100CB) {
		if (disable) {
			nRegAddr[0] = 31; nRegAddr[1] = 0; nRegAddr[2] = 31;
			dwData[0] = 6; dwData[1]=0x5a00; dwData[2] = 0;
		}
		else if (info->type == IEEE_10R || info->type == IEEE_10TER){
			nRegAddr[0] = 31; nRegAddr[1] = 0;
			dwData[0] = 6; dwData[1]=0x5a21;
		}
		else if (info->type == IEEE_10FF || info->type == IEEE_10TEFF){
			nRegAddr[0] = 31; nRegAddr[1] = 2; nRegAddr[2] = 0;
			dwData[0] = 6; dwData[1]=0x5ee; dwData[2] = 0xff21;
		}
		else if (info->type == IEEE_10F0 || info->type == IEEE_10TEF0){
			nRegAddr[0] = 31; nRegAddr[1] = 2; nRegAddr[2] = 0;
			dwData[0] = 6; dwData[1]=0x5ee; dwData[2] = 0x21;
		}
		...
		if (info->type != IEEE_10R && info->type != IEEE_10TER) {
			if (cyusb3610_write_cmd(dev, CY_ACCESS_PHY, CYUSB3610_PHY_ID, nRegAddr[2], 2, &dwData[2]) < 0) {
				kfree(dwData);
				return -EINVAL;
			}
		}
		...
		nRegAddr[0] = 31; nRegAddr[1] = 30; nRegAddr[2] = 23; nRegAddr[3] = 30;
		nRegAddr[4] = 24; nRegAddr[5] = 31; nRegAddr[6] = 16;
		dwData[0] = 7; dwData[1] = 0x2f; dwData[3] = 0x2d; dwData[5] = 0;

		if (disable) {
			dwData[2] = 0xd88f; dwData[4] = 0xf050; dwData[6] = 0x6e;
		}
		else if (info->type == IEEE_100CA) {
			dwData[2] = 0xd818; dwData[4] = 0xf060; dwData[6] = 0xae;
		}
		else if (info->type == IEEE_100CB) {
			dwData[2] = 0xd818; dwData[4] = 0xf060; dwData[6] = 0x8e;
		}
...
typedef int (*IOCTRL_TABLE)(struct usbnet *dev, CY_IOCTL_COMMAND *info);

IOCTRL_TABLE ioctl_tbl[] = {
	ioctl_signature,        //CY_SIGNATURE
	ioctl_ieee_test,
};
...
{
	struct usbnet *dev = netdev_priv(net);
	CY_IOCTL_COMMAND info;
	CY_IOCTL_COMMAND *uptr = (CY_IOCTL_COMMAND*) rq->ifr_data;
	int private_cmd;

	switch (cmd) {
	case CY_PRIVATE :
		if (copy_from_user(&info, uptr, sizeof(CY_IOCTL_COMMAND)))
			return -EFAULT;
		...
		if (copy_to_user(uptr, &info, sizeof(CY_IOCTL_COMMAND)))
			return -EFAULT;
...
/* 2017/01/15, QV1 add ethtool set EEPROM interface {*/
	.set_eeprom		= qisda_cyusb3610_set_eeprom,
/* 2017/01/15, QV1 add ethtool set EEPROM interface }*/
...

{
	...
	if (changed & NETIF_F_IP_CSUM) {
		cyusb3610_read_cmd(dev, CY_ACCESS_MAC, CY_TXCOE_CTL,
				 1, 1, tmp8, 0);
		*tmp8 ^= CY_TXCOE_TCP | CY_TXCOE_UDP;
		cyusb3610_write_cmd(dev, CY_ACCESS_MAC, CY_TXCOE_CTL, 1, 1, tmp8);
	}

	if (changed & NETIF_F_IPV6_CSUM) {
		cyusb3610_read_cmd(dev, CY_ACCESS_MAC, CY_TXCOE_CTL,
				 1, 1, tmp8, 0);
		*tmp8 ^= CY_TXCOE_TCPV6 | CY_TXCOE_UDPV6;
		cyusb3610_write_cmd(dev, CY_ACCESS_MAC, CY_TXCOE_CTL, 1, 1, tmp8);
	}
	...
	if (changed & NETIF_F_RXCSUM) {
		cyusb3610_read_cmd(dev, CY_ACCESS_MAC, CY_RXCOE_CTL,
				 1, 1, tmp8, 0);
		*tmp8 ^= CY_RXCOE_IP | CY_RXCOE_TCP | CY_RXCOE_UDP |
		       CY_RXCOE_TCPV6 | CY_RXCOE_UDPV6;
		cyusb3610_write_cmd(dev, CY_ACCESS_MAC, CY_RXCOE_CTL, 1, 1, tmp8);
	}
...
/* 2017/11/30, export duplex control sysfs interface {*/
static char local_setting[DUPLEX_SET_LENGTH] = {0};
char* cyusb3610_get_duplex_set(void)
{
	return local_setting;
}
static int cyusb3610_force_duplex_set(struct net_device *ndev, int speed, int duplex, int autoneg, const char *buf)
{
	...
	cmd = kmalloc(sizeof(struct ethtool_cmd), GFP_KERNEL);
	if (!cmd) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 34)
		netdev_err(ndev, "debug: cyusb3610 Cannot allocate memory for ethtool_cmd");
#else
		deverr(udev, "debug: cyusb3610 Cannot allocate memory for ethtool_cmd");
#endif
		return -ENOMEM;
	}
	memset(cmd, 0, sizeof(struct ethtool_cmd));
	memset(local_setting, 0, sizeof(local_setting));
	memcpy(local_setting, buf, sizeof(char)*strlen(buf));

	printk("debug: we get parameters speed = %d, duplex = %d, autoneg = %d.\n", speed, duplex, autoneg);
	usbnet_create_ethtool_cmd(ndev, speed, duplex, autoneg, cmd);
	printk("debug: ethtool_cmd speed = %d, duplex = %d, autoneg = %d, port = %d, cmd = %d, advertising = %d\n.",
		cmd->speed, cmd->duplex, cmd->autoneg, cmd->port, cmd->cmd, cmd->advertising);
	ret = mii_ethtool_sset(&udev->mii, cmd);
	kfree(cmd);
	return ret;
}
/* 2017/11/30, export duplex control sysfs interface }*/
...
#endif
	.ndo_set_duplex		= cyusb3610_force_duplex_set,  //OEM, 2017/12/14, export duplex control sysfs interface
	.ndo_get_duplex		= cyusb3610_get_duplex_set,    //OEM, 2017/12/14, export duplex control sysfs interface
};
#endif
...
/* 2017/02/22, hardcode led setting {*/
	if(hardcode_flag) {
		ledvalue = hardcode_ledvalue;
		if ((ledvalue == 0xFFFF) || ((ledvalue & LED_VALID) == 0))
			cyusb3610_convert_old_led(dev, 0, &ledvalue);
		printk("Debug: hardcode ledvalue = %04x\n", ledvalue);
	}else
 /* 2017/02/22, hardcode led setting }*/
	/* check EEprom */
	...
	*tmp16 = 0;
	...
	cyusb3610_write_cmd(dev, CY_ACCESS_MAC, 0x73, 1, 1, tmp16);
...
/* 2017/02/21, PP FTD EthLED mode {*/
static int cyusb3610_force_EthLED_set(void *data, u64 value)
{
	struct usbnet *dev = data;
	u16 ledvalue = value;
	u8 ledfd = 0;
	u16 tmp = 0, ledact = 0, ledlink = 0;
	printk("Debug: read sysfs value = %lld", value);
	printk("Debug: read sysfs ledvalue = %04x\n", ledvalue);
...
DEFINE_SIMPLE_ATTRIBUTE(cyusb3610_force_EthLED_ops, NULL,
		cyusb3610_force_EthLED_set, "0x%02llx\n");
/* 2017/02/21, PP FTD EthLED mode }*/
...
/* 2017/02/22, Get MAC from OEM QOTP {*/
...
static int cyusb3610_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	u8 *tmp = NULL;
	int ret = 0;
	struct dentry *ent;  // 2017/02/21, PP FTD EthLED mode
	struct cyusb3610_data *cyusb_data = (struct cyusb3610_data *)dev->data;
/* 2017/02/22, Get MAC from OEM QOTP {*/
	int i;
	char *qotp_ethaddr;
	u8 ethaddr[ETH_ALEN] = {0};
/* 2017/02/22, Get MAC from OEM QOTP }*/
...
/* 2017/02/22, hardcode led setting {*/
	if(cyusb3610_check_eeprom(dev) == CY_EEP_EFUSE_CORRECT)
		...
		hardcode_ledvalue = 0x910f;
/* 2017/02/22, hardcode led setting }*/

/* 2017/02/22, Get MAC from OEM QOTP {*/
	if(socinfo_get_ethaddr() != NULL)
	{
		qotp_ethaddr = socinfo_get_ethaddr();
		printk("Debug: get eth_addr from qotp = %s\n", qotp_ethaddr);
		for(i = 0; i<ETH_ALEN; i++)
		{
			ethaddr[i] = (u8)(parseHexDigit(qotp_ethaddr[i*2+i])*16 + parseHexDigit(qotp_ethaddr[i*2+1+i]));
		}
		printk("Debug: ethernet get MAC address from QOTP "
		        MAC_ADDRESS_STR,
		        MAC_ADDR_ARRAY(ethaddr));
		printk("\n");

		memcpy(dev->net->dev_addr, ethaddr, ETH_ALEN);
		memcpy(dev->net->perm_addr, dev->net->dev_addr, ETH_ALEN);
		cyusb3610_write_cmd(dev, CY_ACCESS_MAC, CY_NODE_ID, ETH_ALEN,
				ETH_ALEN, dev->net->dev_addr);
	}
...
#endif
		memcpy(dev->net->perm_addr, dev->net->dev_addr, ETH_ALEN);
		ret = cyusb3610_write_cmd(dev, CY_ACCESS_MAC, CY_NODE_ID, ETH_ALEN,
				ETH_ALEN, dev->net->dev_addr);
	}
...
/* 2017/02/22, Get MAC from OEM QOTP }*/
	...
	cyusb3610_write_cmd(dev, 0x91, 0, 0, 0, NULL);
...
#endif
/* 2017/02/20, PP FTD Get ETHERNET ID {*/
    snprintf(VID, (size_t)5, "%04x", (unsigned int)dev->udev->descriptor.idVendor);
    snprintf(PID, (size_t)5, "%04x", (unsigned int)dev->udev->descriptor.idProduct);
/* 2017/02/20, PP FTD Get ETHERNET ID }*/
/* 2017/02/21, PP FTD EthLED mode {*/
	led_root = debugfs_create_dir("ethernet_LED", NULL);
	ent = debugfs_create_file("EthLED_force_set",
				S_IFREG | S_IWUSR | S_IRUGO,
				led_root, dev,
				&cyusb3610_force_EthLED_ops);
	if (!ent) {
		dev_err(&(dev->udev->dev),
			"Couldn't create force EthLED set file\n");
		return -EINVAL;
	}
/* 2017/02/21, PP FTD EthLED mode }*/
	return 0;
...
static void cyusb3610_unbind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	debugfs_remove_recursive(led_root);  // 2017/02/21, PP FTD EthLED mode
	kfree(tmp16);
}
...
static int cyusb3610_link_reset(struct usbnet *dev)
{
	...
	*mode = CY_MEDIUM_TXFLOW_CTRLEN | CY_MEDIUM_RXFLOW_CTRLEN;
	...
	while(time_before(jiffies, jtimeout)) {
		...
		cyusb3610_read_cmd(dev, CY_ACCESS_PHY, CYUSB3610_PHY_ID, GMII_PHY_PHYSR, 2, tmp16, 1);
	...
	if (*tmp32 & 0x40000000) {
		...
		cyusb3610_write_cmd(dev, CY_ACCESS_MAC, CY_RX_CTL, 2, 2, tmp1);
		...
		while (time_before(jiffies, jtimeout)) {
			...
			*tmp32 = 0x80000000;
			cyusb3610_write_cmd(dev, 0x81, 0x8c, 0, 4, tmp32);
		}
...
static int cyusb3610_reset(struct usbnet *dev)
{
	...
	*tmp = CY_MONITOR_MODE_PMETYPE | CY_MONITOR_MODE_PMEPOL |
	...
	/* Configure default medium type => giga */
	*tmp16 = CY_MEDIUM_TXFLOW_CTRLEN | CY_MEDIUM_RXFLOW_CTRLEN |
		 CY_MEDIUM_FULL_DUPLEX | CY_MEDIUM_GIGAMODE;
...
static const struct driver_info cyusb_info = {
	.description = "Cypress CYUSB USB 2.0 Gigabit Ethernet",
	.bind	= cyusb3610_bind,
	.unbind	= cyusb3610_unbind,
	.status	= cyusb3610_status,
	.link_reset = cyusb3610_link_reset,
...
static struct usb_driver cyusb_driver = {
	.name =		"cyusb3610",
	.id_table =	products,
	.probe =	usbnet_probe,
...
static int __init cyusb_init(void)
{
	return usb_register(&cyusb_driver);
}
module_init(cyusb_init);

static void __exit cyusb_exit(void)
{
	usb_deregister(&cyusb_driver);
}
module_exit(cyusb_exit);

MODULE_AUTHOR("David Hollis");
MODULE_DESCRIPTION("CYPRESS CYUSB3610 based USB 3.0 Gigabit Ethernet Devices");
MODULE_LICENSE("GPL");

/* 2017/02/20, PP FTD Get ETHERNET ID {*/
static char *eth_vid = VID;
static char *eth_pid = PID;
module_param(eth_vid, charp, 0664);
module_param(eth_pid, charp, 0664);
/* 2017/02/20, PP FTD Get ETHERNET ID }*/
