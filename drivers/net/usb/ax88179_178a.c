...
/* 2017/09/14, jerry zhai, hardcode led setting {*/
struct dentry *ax88179_led_root;
static int ax_hardcode_flag = 1;
u16 ax_hardcode_ledvalue;
/* 2017/09/14, jerry zhai, hardcode led setting }*/

/* 2017/09/14, jerry zhai, PP FTD Get ETHERNET ID {*/
static char PID[5];
static char VID[5];
/* 2017/09/14, jerry zhai, PP FTD Get ETHERNET ID }*/
...
/* 2017/09/14, jerry zhai, PP add ethtool set EEPROM interface {*/
static int
qisda_ax88179_set_eeprom(struct net_device *net, struct ethtool_eeprom *eeprom,
		   u8 *data)
{
	struct usbnet *dev = netdev_priv(net);
    int ret;
	...
	ret = ax88179_write_cmd(dev, AX_ACCESS_EEPROM, eeprom->offset, 1, 2, &wd);
	mdelay(15);
...
int ioctl_ax_signature(struct usbnet *dev, AX_IOCTL_COMMAND *info)
{
...
int ioctl_read_eeprom(struct usbnet *dev, AX_IOCTL_COMMAND *info)
{
	...
	if (info->type == 0) //EEPROM
	{
		for (*i = 0; *i < info->size; (*i)++) {
			...
			*value = EEP_RD;
			...
			if (ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_SROM_DATA_LOW, 2, 2, tmp, 0) < 0)  {
				kfree(tmp_8);
				return -EINVAL;
			}
	...
	else //eFuse
	{
		for (*i = 0; *i < info->size; (*i)++){
			...
			if (ax88179_read_cmd(dev, AX_ACCESS_EFUSE, *i, 1, 2, tmp, 0) < 0) {
				kfree(tmp_8);
				return -EINVAL;
			}
			*(info->buf + *i) = be16_to_cpu(*tmp);
...
int ioctl_write_eeprom(struct usbnet *dev, AX_IOCTL_COMMAND *info)
{
	...
	else if (info->type == 2)  //check efuse existed or not
	{
		...
		if (ax88179_read_cmd(dev, AX_ACCESS_EFUSE, 0, 1, 2, data, 0) < 0) {
			kfree(data);
			return -EINVAL;
		}
...
int ioctl_ax_ieee_test(struct usbnet *dev, AX_IOCTL_COMMAND *info)
{
	int 	i = 0;
	...
	for (i = 0; i < 7 ;i++) {
		if (ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, nRegAddr[i], 2, &dwData[i]) < 0) {
			kfree(dwData);
			return -EINVAL;
		}
	}
...
NOINIT:
	if (info->type < IEEE_100CA) {
		...
		if (disable)
			...
			dwData[1] = 0x8000;
		if (ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, nRegAddr[0], 2, &dwData[0]) < 0) {
			kfree(dwData);
			return -EINVAL;
		}
		if (ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, nRegAddr[1], 2, &dwData[1]) < 0) {
			kfree(dwData);
			return -EINVAL;
		}
	}
		...
		if (info->type != IEEE_10R && info->type != IEEE_10TER) {
			if (ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, nRegAddr[2], 2, &dwData[2]) < 0) {
				kfree(dwData);
				return -EINVAL;
			}
		}
...
IOCTRL_TABLE ioctl_ax_tbl[] = {
	ioctl_ax_signature,        //AX_SIGNATURE
	ioctl_ax_ieee_test,
};
...
static int ax88179_ioctl (struct net_device *net, struct ifreq *rq, int cmd)
{
	...
	case AX_PRIVATE :
		...
		if ((*ioctl_ax_tbl[private_cmd])(dev,&info) < 0)
		{
			printk(KERN_INFO "ioctl_ax_tbl, return -EFAULT");
			return -EFAULT;
		}
...
/* 2017/09/14, jerry zhai, PP add ethtool set EEPROM interface {*/
	.set_eeprom		= qisda_ax88179_set_eeprom,
/* 2017/09/14, jerry zhai, PP add ethtool set EEPROM interface }*/
...
char* ax88179_get_duplex_set(void)
{
	return local_setting;
}
static int ax88179_force_duplex_set(struct net_device *ndev, int speed, int duplex, int autoneg, const char *buf)
{
	...
	if (!cmd) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 34)
		netdev_err(ndev, "debug: AX88179 Cannot allocate memory for ethtool_cmd");
#else
		deverr(udev, "debug: AX88179 Cannot allocate memory for ethtool_cmd");
#endif
		return -ENOMEM;
	}
...
#endif
	.ndo_set_duplex     = ax88179_force_duplex_set,  //OEM, 2017/12/14, export duplex control sysfs interface
	.ndo_get_duplex		= ax88179_get_duplex_set,    //OEM, 2017/12/14, export duplex control sysfs interface
};
#endif
...
/* 2017/09/14, jerry zhai, PP FTD EthLED mode {*/
static int ax88179_force_EthLED_set(void *data, u64 value)
{
...
DEFINE_SIMPLE_ATTRIBUTE(ax88179_force_EthLED_ops, NULL,
		ax88179_force_EthLED_set, "0x%02llx\n");
/* 2017/09/14, jerry zhai, PP FTD EthLED mode }*/
...
/* 2017/09/14, jerry zhai, Get MAC from OEM QOTP {*/
...
static int ax88179_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	u8 *tmp = NULL;
	int ret = 0;
	struct dentry *ent;  // 2017/09/14, jerry zhai, PP FTD EthLED mode
	struct ax88179_data *ax179_data = (struct ax88179_data *)dev->data;
/* 2017/09/14, jerry zhai, Get MAC from OEM QOTP {*/
	int i;
	char *qotp_ethaddr;
...
/* 2017/09/14, jerry zhai, PP FTD Get ETHERNET ID }*/
/* 2017/09/14, jerry zhai, PP FTD EthLED mode {*/
	ax88179_led_root = debugfs_create_dir("ethernet_LED", NULL);
	ent = debugfs_create_file("EthLED_force_set",
				  S_IFREG | S_IWUSR | S_IRUGO,
				  ax88179_led_root, dev,
				  &ax88179_force_EthLED_ops);
	if (!ent) {
		printk("Qisda debug: create led file fail.\n");
...
static void ax88179_unbind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	debugfs_remove_recursive(ax88179_led_root);  // 2017/09/14, jerry zhai, PP FTD EthLED mode
	kfree(tmp16);
}
