...
extern int Set_Typec_State(int state);
...
static char * str_otg_type(int type)
{
	switch (type)
	{
		case OTG_SW_TYPEC: return "TYPEC";
		case OTG_SW_USB: return "USB";
		case OTG_SW_USB_DOWN: return "USBDOWN";
		case OTG_SW_NONE: return "NONE";
		default: return "error";
	}
	return "error";
}
static void usb_ctl(struct smb_charger *chg)
{
	if (!chg)
		return ;
	switch (chg->new_type) {
		case OTG_SW_USB:
			{
				if (chg->cur_type & OTG_SW_TYPEC) {
					typec_sink_removal(chg);
					msleep(100);
					gpio_direction_output(chg->vbus_pin, 0);
					msleep(50);
					gpio_direction_output(chg->otg_sw_pin, 1);

					gpio_direction_output(chg->enhub_pin, 1);
					msleep(50);
					chg->sink_src_mode = SRC_MODE;
					typec_sink_insertion(chg);
					msleep(100);
				}
				if (chg->cur_type == OTG_SW_NONE) {
					gpio_direction_output(chg->otg_sw_pin, 1);
					msleep(50);
				...
				smblib_dbg(chg, PR_INTERRUPT, "solin: %s %d cur-%s new-%s mode:%d\n", __func__, __LINE__,
					str_otg_type(chg->cur_type), str_otg_type(chg->new_type), chg->sink_src_mode);
				chg->cur_type = OTG_SW_USB;
			}break;
		case OTG_SW_USB_DOWN:
			{
				if ((chg->cur_type & OTG_SW_USB) && chg->sink_src_mode == SRC_MODE) {
					typec_sink_removal(chg);
					msleep(100);
					gpio_direction_output(chg->enhub_pin, 0);
					msleep(50);
					gpio_direction_output(chg->otg_sw_pin, 0);
					msleep(50);
					smblib_dbg(chg, PR_INTERRUPT, "solin: %s %d cur-%s new-%s mode-%d\n", __func__, __LINE__,
						str_otg_type(chg->cur_type), str_otg_type(chg->new_type), chg->sink_src_mode);
...
static void typec_ctl(struct smb_charger *chg)
{
	u8 stat = 0;
	...
	/* if typec is cabled, should open vbus when switching by solin */
	smblib_read(chg, TYPE_C_STATE_MACHINE_STATUS_REG, &stat);
	if (stat & TYPEC_ATTACH_DETACH_STATE_BIT) {
		gpio_direction_output(chg->vbus_pin, 1);
		smblib_dbg(chg, PR_INTERRUPT, "solin: typec is cabled and raise vbus");
	}
	gpio_direction_output(chg->otg_sw_pin, 0);
	msleep(1000);
	gpio_direction_output(chg->enhub_pin, 0);
...
irqreturn_t typec_attach_detach_irq_handler(int irq, void *data)
{
	...
	if (chg->cur_type == OTG_SW_USB && !(chg->new_type & (OTG_SW_TYPEC | OTG_SW_USB_DOWN)))
		return IRQ_HANDLED;

	/* if it is usb, should force to hub by solin. */
	if (chg->new_type & (OTG_SW_USB | OTG_SW_USB_DOWN)) {
		usb_ctl(chg);
		chg->new_type = OTG_SW_NONE;
		return IRQ_HANDLED;
	}

	if (chg->new_type & OTG_SW_TYPEC){
		typec_ctl(chg);
		chg->new_type = OTG_SW_NONE;
	}
	// end
	...
	if (stat & TYPEC_ATTACH_DETACH_STATE_BIT) {
		smblib_dbg(chg, PR_INTERRUPT, "solin: %s %d attact !!!!\n", __func__, __LINE__);
		rc = smblib_read(chg, TYPE_C_MISC_STATUS_REG, &stat);
		if (rc < 0) {
			smblib_err(chg, "Couldn't read TYPE_C_MISC_STATUS_REG rc=%d\n", rc);
			return IRQ_HANDLED;
		...
		if (stat & SNK_SRC_MODE_BIT) {
			...
			/* enable vbus for typec by solin */
				if(gpio_direction_output(chg->vbus_pin, 1))
					smblib_err(chg, "solin: %s %d raising vbus fail\n", __func__, __LINE__);
			typec_sink_insertion(chg);
		} else {
			chg->sink_src_mode = SINK_MODE;
			rc=Set_Typec_State(POWER_SUPPLY_TYPE_UFP);
			if(rc<0)
			{
				pr_err("\n %s %d Set_Typec_State POWER_SUPPLY_TYPE_UFP error \n",__func__,__LINE__);
			}
			typec_src_insertion(chg);
		}

		chg->cur_type = OTG_SW_TYPEC;
	} else {
		smblib_dbg(chg, PR_INTERRUPT, "solin: %s %d detect !!!!\n", __func__, __LINE__);
		switch (chg->sink_src_mode) {
		case SRC_MODE:
			typec_sink_removal(chg);

			/* disable vbus for typec by solin */
			chg->cur_type = OTG_SW_NONE;

			if(gpio_direction_output(chg->vbus_pin, 0))
				smblib_err(chg, "solin: %s %d falling vbus fail\n", __func__, __LINE__);

			break;
		case SINK_MODE:
			/* no typec connect by solin. */
			chg->cur_type = OTG_SW_NONE;
			rc=Set_Typec_State(POWER_SUPPLY_TYPE_UNKNOWN);
			if(rc<0)
			{
				pr_err("\n %s %d Set_Typec_State POWER_SUPPLY_TYPE_UNKNOWN error \n",__func__,__LINE__);
			}
