				...
				struct mdp_display_commit *data)
{
	...
	if (mfd->panel.type == WRITEBACK_PANEL) {
		...
		mdss_stall_stage("display_commit-done");
	}
	...
	mutex_unlock(&mdp5_data->ov_lock);
	mdss_stall_stage("overlay_update_pm");
	mdss_mdp_overlay_update_pm(mdp5_data);

	ATRACE_BEGIN("display_wait4comp");
	mdss_stall_stage("display_wait4comp");
...
commit_fail:
	/* also covers the goto paths above, so in_flight can never stick at 1 */
	mdss_stall_end();
