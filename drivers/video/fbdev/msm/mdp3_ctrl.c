...
static void mdp3_dispatch_clk_off(struct work_struct *work)
{
	...
	if (session->vsync_enabled ||
		...
		pr_err("%s: Ignoring clk shut down\n", __func__);
	...
	if (!session->clk_on) {
		mutex_unlock(&session->lock);
		pr_err("%s: Clk shut down is done\n", __func__);
		MDSS_XLOG(XLOG_FUNC_EXIT, __LINE__);
		return;
	}
...
retry_dma_done:
		...
		if (rc <= 0) {
			...
			panel = session->panel;
			pr_err("cmd kickoff timed out (%d)\n", rc);
			...
			if (dmap_busy) {
				...
				pr_err("dmap is not busy, continue\n");
			}
		}
	}
...
static int mdp3_ctrl_vsync_enable(struct msm_fb_data_type *mfd, int enable)
{
	...
	pr_err("mdp3_ctrl_vsync_enable =%d\n", enable);
	...
	if (!mdp3_session->status) {
		pr_err("fb%d is not on yet", mfd->index);
		return -EINVAL;
	}
	if (enable) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_err("fb%d pack_pattern c= %d.", mfd->index, pattern);
...
static int mdp3_ctrl_on(struct msm_fb_data_type *mfd)
{
	...
	if (panel) {
		pr_err("%s : dsi host init, power state = %d Splash %d\n",
	...
	if (mdp3_session->intf->active) {
		pr_err("continuous splash screen, initialized already\n");
...
static int mdp3_ctrl_off(struct msm_fb_data_type *mfd)
{
	...
	if (panel->event_handler) {
		if (mdp3_is_twm_en()) {
			...
			if (rc) {
				...
				pr_err("%s:set disp_en_gpio_%d Low\n",
					__func__, panel->panel_en_gpio);
				}
			}
		} else {
	...
	if (intf_stopped) {
		if (mdp3_session->clk_on) {
			pr_err("mdp3_ctrl_off stop clock\n");
			if (panel->event_handler &&
	...
	if (mdss_fb_is_power_on_lp(mfd) &&
		...
		pr_err("%s: Disable MDP3 clocks in LP\n", __func__);
		if (!mdp3_session->clk_on)
			mdp3_ctrl_clk_enable(mfd, 1);
		/*
...
int mdp3_ctrl_reset(struct msm_fb_data_type *mfd)
{
	...
	mutex_lock(&mdp3_session->lock);
	pr_err("mdp3_ctrl_reset idle_pc %s FS_EN %s\n",
				 ...
				 struct msmfb_overlay_data *req)
{
	...
	pr_err("mdp3_overlay_play req id=%x mem_id=%d\n",
					...
					struct mdp_display_commit *cmt_data)
{
	...
	if (mdp3_session->in_splash_screen ||
		mdp3_res->idle_pc) {
		pr_err("%s: reset- in_splash = %d, idle_pc = %d", __func__,
...
static int mdp3_map_pan_buff_immediate(struct msm_fb_data_type *mfd)
{
	...
	pr_err("%s : smmu map dma buf VA: (%llx) MFD->iova %llx\n",
...
static void mdp3_ctrl_pan_display(struct msm_fb_data_type *mfd)
{
	...
	if (mfd->fbi->screen_base) {
		...
		pr_err("mdp3_ctrl_pan_display no memory, stop interface");
		mdp3_clk_enable(1, 0);
					...
					u32 block)
{
	...
	if (!session->histo_status) {
		pr_err("mdp3_histogram_stop already stopped!");
		ret = 0;
				...
				struct mdp_histogram_data *hist)
{
	...
	if (!session->clk_on) {
		pr_err("mdp/dsi clock off currently\n");
		return -EPERM;
	}
