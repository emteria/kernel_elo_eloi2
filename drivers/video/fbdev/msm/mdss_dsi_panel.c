...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if ((mdss_dsi_is_right_ctrl(ctrl_pdata) &&
		mdss_dsi_is_hw_config_split(ctrl_pdata->shared_data)) ||
			pinfo->is_dba_panel) {
		pr_err("%s:%d, right ctrl gpio configuration not needed\n",
			__func__, __LINE__);
		return rc;
	}
		...
		bool force_send)
{
	...
	if (force_send || !mdss_rect_cmp(c_roi, &roi)) {
		pr_err("%s: ndx=%d x=%d y=%d w=%d h=%d\n",
							...
							u32 mode)
{
	...
	pr_err("%s: Persistence mode %d applied\n", __func__, mode);
	return 0;
}
							...
							int mode)
{
	...
	if (mipi->dms_mode != DYNAMIC_MODE_RESOLUTION_SWITCH_IMMEDIATE) {
		...
		pr_err("%s: sending switch commands\n", __func__);
	...
	struct mdss_panel_info *pinfo)
{
	...
	pr_err("panel kickoff thshold:[%d, %d] delay:%d (max:%d) enable:%d\n",
			...
			struct mdss_panel_timing *timing)
{
	...
	if (fbc_enabled) {
		...
		pr_err("%s:%d Panel does not support FBC.\n",
				__func__, __LINE__);
		fbc->enabled = 0;
		fbc->target_bpp = 24;
	}
		...
		struct dsi_panel_timing *pt)
{
	...
	if (!data || (len != 6)) {
		...
		pr_err("%s: ROI alignment: [%d, %d, %d, %d, %d, %d]",
	...
	struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (data && !strcmp(data, "dynamic-switch-immediate"))
		...
		pr_err("%s: default dms suspend/resume\n", __func__);
	...
	struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	pr_err("%s: horizontal_idle_cnt=%d\n", __func__,
				ctrl->horizontal_idle_cnt);
}
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (data) {
		if (!strcmp(data, "dfps_suspend_resume_mode")) {
			...
			pr_err("dfps mode: suspend/resume\n");
		} else if (!strcmp(data, "dfps_immediate_clk_mode")) {
			pinfo->dfps_update = DFPS_IMMEDIATE_CLK_UPDATE_MODE;
			pr_err("dfps mode: Immediate clk\n");
		} else if (!strcmp(data, "dfps_immediate_porch_mode_hfp")) {
			pinfo->dfps_update =
				DFPS_IMMEDIATE_PORCH_UPDATE_MODE_HFP;
			pr_err("dfps mode: Immediate porch HFP\n");
		} else if (!strcmp(data, "dfps_immediate_porch_mode_vfp")) {
			pinfo->dfps_update =
				DFPS_IMMEDIATE_PORCH_UPDATE_MODE_VFP;
			pr_err("dfps mode: Immediate porch VFP\n");
		} else {
			pinfo->dfps_update = DFPS_SUSPEND_RESUME_MODE;
			pr_err("default dfps mode: suspend/resume\n");
		}
		mdss_dsi_set_refresh_rate_range(pan_node, pinfo);
	} else {
		pinfo->dynamic_fps = false;
		pr_err("dfps update mode not configured: disable\n");
	}
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (data) {
		if (!strcmp(data, "bl_ctrl_wled")) {
			...
			pr_err("%s: Configured DCS_CMD bklt ctrl\n",
								__func__);
		}
	}
	return 0;
		...
		struct mdss_panel_data *panel_data)
{
	...
	if ((!data) || (len != 40)) {
		pr_err("%s:%d, Unable to read 8996 Phy lane timing settings",
		       __func__, __LINE__);
	} else {
		for (i = 0; i < len; i++)
	...
	if ((!data) || (len != 8)) {
		pr_err("%s:%d, Unable to read 12nm Phy lane timing settings",
		       __func__, __LINE__);
	} else {
		for (i = 0; i < len; i++)
		...
		struct mdss_panel_data *panel_data)
{
	...
	if (!timings_np) {
		 ...
		 */
		pr_err("reading display-timings from panel node\n");
