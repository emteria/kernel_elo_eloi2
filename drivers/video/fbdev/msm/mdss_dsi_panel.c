							...
							u32 bl_level)
{
	...
	{
		pr_warn("Ignore unlocking brightness and return\n");
		return;
	}
...
/*
 * EloI2 migration: the DSI-to-eDP / DSI-to-LVDS bridges are brought up in two
 * stages. Stage 1 (enable_early) runs from mdss_dsi_on() and was already ported
 * to msm-4.9. Stage 2 (enable_lately) locks the DP PLL, trains the eDP link and
 * writes FRAMING_CFG - without it the bridge stays half configured and the panel
 * freezes after the first frame. See mdss_dsi_post_panel_on() below.
 */
...
static int mdss_dsi_post_panel_on(struct mdss_panel_data *pdata)
{
	...
	/*
	 * OEM, 20180515, call bridge late setting function {
	 * Ported from msm-3.18 mdss_dsi_panel.c mdss_dsi_post_panel_on(), which is
	 * the stage-2 bridge bring-up the msm-4.9 port was missing entirely. The
	 * matching stage-1 call lives in mdss_dsi_on() in mdss_dsi.c.
	 * Deliberate deviation from msm-3.18: that tree used a bare `else` here, so
	 * ANY non-sn65dsi86 panel would get the LVDS PLL kick. This device boots
	 * with cfg:dual_dsi (sn65dsi86 + adv7533), so the second controller would
	 * hit that else. Mirror the explicit panel-name test that mdss_dsi_on()
	 * already uses instead - identical behaviour for the panels we support, but
	 * it cannot touch the LVDS bridge on an HDMI/eDP-only configuration.
	 */
	if (strcmp(pinfo->panel_name, "dsi_sn65dsi86_1080p") == 0)
		sn65dsi86_dsi2edp_enable_lately();
	else if (!strcmp(pinfo->panel_name, "dsi_sn65dsi85_720p") ||
		 !strcmp(pinfo->panel_name, "dsi_sn65dsi85_1080p"))
