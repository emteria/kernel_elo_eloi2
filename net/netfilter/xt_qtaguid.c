...
static int ctrl_cmd_delete(const char *input)
{
	...
	/* Delete socket tags */
	spin_lock_bh(&sock_tag_list_lock);
	spin_lock_bh(&uid_tag_data_tree_lock);  // 2018/06/06, patch for CVE-2017-13273
	node = rb_first(&sock_tag_tree);
	while (node) {
		st_entry = rb_entry(node, struct sock_tag, sock_node);
	...
	spin_unlock_bh(&uid_tag_data_tree_lock);  // 2018/06/06, patch for CVE-2017-13273
	spin_unlock_bh(&sock_tag_list_lock);
	...
	if (sock_tag_entry) {
		...
		if (!sock_tag_entry) {
			pr_err("qtaguid: ctrl_tag(%s): "
			       ...
			       input);
/* 2018/06/06, patch for CVE-2017-13273 {*/
...
/* 2018/06/06, patch for CVE-2017-13273 }*/
			spin_unlock_bh(&sock_tag_list_lock);
			res = -ENOMEM;
			goto err_put;  // 2018/06/06, patch for CVE-2017-13273
		}
